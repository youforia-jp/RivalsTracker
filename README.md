...
│
├── IGNs.csv                    # ⚠️ YOUR INPUT — add player IGNs here (gitignored)
├── IGN_stats.csv               # ⚠️ YOUR OUTPUT — generated by the script (gitignored)
│
├── ign_SAMPLE.csv              # Example input showing format (safe to commit)
├── IGN_stats_SAMPLE.csv        # Example output showing format (safe to commit)
│
├── requirements.txt            # Python dependencies
├── .gitignore                  # Excludes IGNs.csv and IGN_stats.csv from git
└── README.md                   # You are here
```

> ⚠️ **`IGNs.csv` and `IGN_stats.csv` are gitignored** — your real player data never leaves your machine.
...
Create a file called `IGNs.csv` in the same folder as `Tracker.py`. It's a single-column CSV — just IGNs, nothing else needed.
...
See [`ign_SAMPLE.csv`](ign_SAMPLE.csv) for a reference example with 20 known players.
...
```bash
# Reads IGNs.csv, writes IGN_stats.csv
python Tracker.py
```
...
| File | Description |
|------|-------------|
| [`ign_SAMPLE.csv`](ign_SAMPLE.csv) | Example input — 20 known Marvel Rivals players/streamers with public profiles |
| [`IGN_stats_SAMPLE.csv`](IGN_stats_SAMPLE.csv) | Example output showing the full 3-section format |

> Sample output contains fabricated numbers for demo purposes. Run the pipeline with real IGNs to get live data.
